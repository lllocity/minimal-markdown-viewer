...
- **実装を追加・変更したときは必ずテストも追加または修正する**
- テスト対象: ロジック層（`DocumentViewModel` など）。View のテストは不要
- テストファイル: `Minimal Markdown ViewerTests/` 以下
- テスト実行: **⌘+U** で全テストが緑（合格）になることを確認してからコミットする
- 新しいエラーケースを追加したら `DocumentViewModelTests.swift` に対応するテストケースを追加する
...
- **Web 版（`web/`・Next.js 16）を開発中。** ステップ一覧と残タスクは [docs/web/ROADMAP.md](./docs/web/ROADMAP.md) を参照（W01〜W09 完了・本番公開中 https://minimal-markdown-viewer-seven.vercel.app / 残: W10 フォルダブックマーク）。別セッションで再開するときはまずこのロードマップを見ること。
