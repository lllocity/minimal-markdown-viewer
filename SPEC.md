...
>
> 📌 **今後の方針**: この機能差異は暫定的なもので、**将来的に iOS 版を Web 版に寄せる**
> （iOS 版にも Google ログイン・Drive ブラウズ等を取り込む）ことで解消していく。
> Web 版が先行して認証・ブラウズを実装しており、iOS 版がそれに追従する形を想定する。
...
> 📌 ファイルブラウザ・認証を OUT にしているのは**現状の割り切り**。将来的には
> **iOS 版を Web 版に寄せて**これらを取り込み、機能差異を解消していく方針（第1部「3. スコープの考え方」参照）。
